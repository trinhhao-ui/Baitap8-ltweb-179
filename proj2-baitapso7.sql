...
-- ============================================================
IF NOT EXISTS (
    SELECT name FROM sys.databases WHERE name = N'webst2'
)
BEGIN
    CREATE DATABASE webst2 COLLATE Vietnamese_CI_AS;
    PRINT N'Database webst2 đã được tạo.';
END
ELSE
BEGIN
    PRINT N'Database webst2 đã tồn tại, bỏ qua bước tạo.';
END
GO
...
BEGIN
    ...
    -- Xóa cột images cũ nếu còn tồn tại
    IF EXISTS (
    ...
    BEGIN
        ...
        PRINT N'  → Đã thêm cột category_id.';

        -- Thêm FK sau khi thêm cột
        IF NOT EXISTS (
        ...
        BEGIN
            ALTER TABLE products
                ...
                FOREIGN KEY (category_id) REFERENCES categories(id)
                ON DELETE SET NULL;
            PRINT N'  → Đã thêm FK category_id → categories(id).';
        END
    END
    ELSE
    BEGIN
...
-- ============================================================
-- 4. Tạo bảng product_images (quan hệ 1-N với products)
-- ============================================================
IF NOT EXISTS (
...
ELSE
BEGIN
    PRINT N'Bảng categories đã có dữ liệu, bỏ qua.';
END
GO

-- ============================================================
-- 6. Dữ liệu mẫu cho products (10 sản phẩm, có category_id)
-- ============================================================
...
ELSE
BEGIN
    PRINT N'Bảng products đã có dữ liệu, bỏ qua.';
END
GO
